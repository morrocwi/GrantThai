...
project_content_sha256: 5541ac9390361c269dd69a34cbbd983c3c0573abffd30eef6bfb38a5f1cf39f7
project_state_sha256: ab484118cf24d3ab72d1f7e31d33af58cd77a11d3282278cade8b5093661d27a
...
# NRIIS submission worksheet: FICTIONAL-LECTURER-EXAMPLE-0001
...
- Findings: BLOCK 0 / REVIEW 11 / INFO 31
...
- **W102** [CORE.GENERAL.KEYWORDS_EN]: CORE.GENERAL.KEYWORDS_EN: 2 items is below the writing target of 3 items. Basis: SD-5 p3 (at most 5 keywords, FF full-proposal form 2570); proposed_default minimum 3; NEEDS_VERIFICATION for other forms and editions. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.GENERAL.KEYWORDS_TH]: CORE.GENERAL.KEYWORDS_TH: 2 items is below the writing target of 3 items. Basis: SD-5 p3 (at most 5 keywords, FF full-proposal form 2570); proposed_default minimum 3; NEEDS_VERIFICATION for other forms and editions. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.NARRATIVE.FRAMEWORK]: CORE.NARRATIVE.FRAMEWORK: 10 words is below the writing target of 50 words. Basis: proposed_default; NEEDS_VERIFICATION. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.NARRATIVE.METHOD]: CORE.NARRATIVE.METHOD: 8 words is below the writing target of 100 words. Basis: proposed_default; NEEDS_VERIFICATION. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.NARRATIVE.RATIONALE]: CORE.NARRATIVE.RATIONALE: 13 words is below the writing target of 100 words. Basis: registry:CORE.NARRATIVE.RATIONALE guidance (observed instruction, not more than 3,000 words); NEEDS_VERIFICATION. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.NARRATIVE.SUMMARY]: CORE.NARRATIVE.SUMMARY: 27 words is below the writing target of 100 words. Basis: registry:CORE.NARRATIVE.SUMMARY guidance (observed instruction, not more than 3,000 words); SD-5 p3 (FF full-proposal form 2570, at most 3000 words, read at v0.2 integration); NEEDS_VERIFICATION for other forms and editions. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.NARRATIVE.THEORY]: CORE.NARRATIVE.THEORY: 12 words is below the writing target of 50 words. Basis: proposed_default; NEEDS_VERIFICATION. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.RESEARCH.GAP]: CORE.RESEARCH.GAP: 17 words is below the writing target of 30 words. Basis: proposed_default; core/02 §4A.2 compression target NEEDS_VERIFICATION (source excluded from the public repo). Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.RESEARCH.NATIONAL_NEED]: CORE.RESEARCH.NATIONAL_NEED: 19 words is below the writing target of 40 words. Basis: proposed_default; core/02 §4A.2 compression target NEEDS_VERIFICATION (source excluded from the public repo). Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [CORE.RESEARCH.PROBLEM]: CORE.RESEARCH.PROBLEM: 18 words is below the writing target of 40 words. Basis: proposed_default; core/02 §4A.2 compression target NEEDS_VERIFICATION (source excluded from the public repo). Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
- **W102** [METHOD.PLAN.ANALYSIS]: METHOD.PLAN.ANALYSIS: 13 words is below the writing target of 15 words. Basis: proposed_default; NEEDS_VERIFICATION. Next step: Check the micro-template (grantthai explain FIELD_ID) for the parts a reader expects.
...
- `CORE.ALIGNMENT.POLICY_PATHWAY`: NEEDS_VERIFICATION. Next step: confirm this value against a current, cited source.
...
```text
FICTIONAL Plan: community resources
```
...
```text
FICTIONAL issue: local water governance
```
...
```text
ตัวอย่างสมมติ: การจัดตารางใช้น้ำร่วมกันของชุมชนตัวอย่าง
```
...
```text
FICTIONAL: Shared water-use scheduling in an example community
```
...
- VALIDATION: no BLOCK/REVIEW finding
- AUTHORED_BY: human (self-declared)
- ARITHMETIC CHECK: value 50000.00 vs sum of budget lines 50000.00: OK
...
```text
FICTIONAL: This project contributes to KR1 by co-designing a written water-use schedule, producing a community guide for use by reservoir-sharing households.
```
...
|---|---|---|---|---|---|---|---|---|
| TM1 | FICTIONAL Lecturer A | FICTIONAL Example University | PI | 60 | FICTIONAL: registered (NEEDS_VERIFICATION) |  | design, analysis, report | {"highest_degree": "FICTIONAL doctorate"} |
| TM2 | FICTIONAL Lecturer B | FICTIONAL Example University | CO_RESEARCHER | 40 | FICTIONAL: registered (NEEDS_VERIFICATION) |  | fieldwork, community workshops |  |
...
| id | person_id | position | expertise | special_interests | ongoing_projects |
|---|---|---|---|---|---|
| TEX1 | TM1 | FICTIONAL lecturer | community resource governance, survey methods |  |  |
...
```text
FICTIONAL summary. The example community shares one small reservoir without an agreed schedule. The project documents current use, then co-designs and tests a written schedule with households.
```
...
```text
FICTIONAL rationale, rendered from the Need, Problem, Gap and primary research question records.
```
...
```text
FICTIONAL: (1) describe current water use; (2) co-design and test a shared schedule.
```
...
```text
FICTIONAL framework: agreed rules -> predictable access -> fewer disputes.
```
...
```text
FICTIONAL theory note: common-pool resource rules, stated in the lecturer's own words.
```
...
```text
FICTIONAL method narrative, rendered from the METHOD.PLAN.* records.
```
...
```text
FICTIONAL Author A. (2024). A synthetic review of small-reservoir management. FICTIONAL Journal of Examples, 1(1), 1-10.
FICTIONAL Author B. (2023). Synthetic case study of shared irrigation schedules. FICTIONAL Proceedings, 2, 20-30.
```
...
| key | value |
|---|---|
| check_status | CHECKED_NONE_RELATED |
| search_sources | FICTIONAL public patent database search |
| related_ip |  |
| notes |  |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| ACT1 | 1 | Prepare instruments and consent | 1, 2 | 20 | OBJ1 | MP1 |  | BI3 |  | TM1 |  |
| ACT2 | 1 | Baseline survey and co-design workshops | 3, 4, 5, 6, 7 | 40 | OBJ1, OBJ2 | MP1, MP2 |  | BI1, BI2 | ACT1 | TM2 |  |
| ACT3 | 1 | Follow-up survey, analysis and report | 8, 9, 10, 11, 12 | 40 | OBJ2 |  | OUT1, OUT2 |  | ACT2 | TM1 |  |
...
|---|---|---|---|---|---|---|
| RSK1 | FICTIONAL: households decline to take part in the workshops. | project delivery | medium | high | Schedule workshops with the community committee; offer two dates. | ACT2 |
...
|---|---|---|---|---|---|---|
| SITE1 | village | FICTIONAL country | FICTIONAL Province A | FICTIONAL Village 1 | fieldwork and workshops | ACT2 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| BI1 | 1 | FICTIONAL: operating | FICTIONAL: compensation |  | FICTIONAL: field assistant fee | Two field assistants, six months. | 1 | assistant-month | 2 | 6 | 3000 | 36000 | ACT2 |  | Survey and workshop support. |
| BI2 | 1 | FICTIONAL: operating | FICTIONAL: materials |  | FICTIONAL: printed questionnaires | Printed questionnaires for 30 households. | 1 | set | 30 | 1 | 150 | 4500 | ACT2 |  | One set per household. |
| BI3 | 1 | FICTIONAL: operating | FICTIONAL: operating costs |  | FICTIONAL: instrument pilot | Pilot of the questionnaire. | 1 | pilot | 1 | 1 | 9500 | 9500 | ACT1 |  | Needed before the baseline survey. |
...
- CONFLICTS: CX-03 (OPEN; see section 4.4)
- AUTHORED_BY: human (self-declared)
- ARITHMETIC CHECK: BI1: 1 x 2 x 6 x 3000 = 36000.00; line_total 36000.00: OK; BI2: 1 x 30 x 1 x 150 = 4500.00; line_total 4500.00: OK; BI3: 1 x 1 x 1 x 9500 = 9500.00; line_total 9500.00: OK; sum(line_total) = 50000.00
...
|---|---|
| applicable | true |
| status | FICTIONAL: to be submitted to the institution's committee |
| committee | FICTIONAL Example University ethics committee |
| protocol_number |  |
| approval_date |  |
| attachment_ids | ATT1 |
...
|---|---|---|---|---|---|---|
| USR1 | community organisation | FICTIONAL community water committee | 1 | committee | Adopt and maintain the dry-season schedule. | OUT2 |
...
|---|---|---|---|---|---|---|---|---|---|
| OUT1 | FICTIONAL: research report | 1 | report | Report on use patterns and the tested schedule. | OBJ1 | ACT3 |  |  |  |
| OUT2 | FICTIONAL: community guide | 1 | guide | A two-page schedule guide for the community. | OBJ2 | ACT3 |  | KR1 | primary |
...
|---|---|---|---|---|---|---|
| PW1 | OUT2 | The committee adopts the guide at its dry-season meeting. |  |  |  | USR1 |
...
|---|---|---|---|---|---|---|---|---|
| OC1 | FICTIONAL: fewer reported water-use disputes in the dry season. | reported disputes per dry season |  |  |  | PW1 | behavioral outcome | KR1 |
...
|---|---|---|---|---|---|---|---|
| ATT1 | ethics-approval.pdf | FICTIONAL: ethics approval | pdf | Standards | UNKNOWN | false |  |
...
### 4.2 Source manifest
- `SRC-1` (OFFICIAL_DOCUMENT): FICTIONAL Provincial Office. (2025). FICTIONAL annual report on community water use. Synthetic document.; locator: Table 3; cited by: CORE.RESEARCH.PROBLEM
- `SRC-2` (PUBLISHED_LITERATURE): FICTIONAL Author A. (2024). A synthetic review of small-reservoir management. FICTIONAL Journal of Examples, 1(1), 1-10.; locator: Section 4; cited by: CORE.NARRATIVE.REFERENCES, CORE.PRIORKNOWLEDGE.PK1, CORE.RESEARCH.THEORETICAL_FOUNDATIONS
- `SRC-3` (PUBLISHED_LITERATURE): FICTIONAL Author B. (2023). Synthetic case study of shared irrigation schedules. FICTIONAL Proceedings, 2, 20-30.; cited by: CORE.NARRATIVE.REFERENCES, CORE.EVIDENCE.E1
...
- **CX-01** (OPEN): Research Core and Methodology: authoring fields or observed NRIIS fields?
  ...
  - In this project: CORE.RESEARCH.GAP, CORE.RESEARCH.NATIONAL_NEED, CORE.RESEARCH.OBJECTIVES, CORE.RESEARCH.PROBLEM, CORE.RESEARCH.RQ.PRIMARY, CORE.RESEARCH.THEORETICAL_FOUNDATIONS, METHOD.PLAN.ANALYSIS, METHOD.PLAN.DATA_COLLECTION, METHOD.PLAN.DESIGN, METHOD.PLAN.ETHICS, METHOD.PLAN.INSTRUMENTS, METHOD.PLAN.SAMPLE
...
- **CX-04** (OPEN): Does Impact follow Beneficiary, or do both follow Outcome?
  ...
  - In this project: no record of this project (affects spec/common/chain.yaml, RESULTS.CHAIN.BENEFICIARIES, RESULTS.CHAIN.IMPACTS)
...
`CORE.ALIGNMENT.POLICY_PATHWAY` (ORIGIN: AUTHORING_CORE; chain: fields; status: DRAFT; not on a tab because: authoring core, an internal research-design record (it reaches NRIIS only through the narrative box it feeds, if any))
...
```text
{
 "owner": "FICTIONAL community water committee",
 "routine": "Review the schedule at the start of each dry season.",
 "continuity_mechanism": "The committee keeps the printed guide and updates it.",
 "user_ids": [
  "USR1"
 ]
}
...
```text
{
 "statement": "FICTIONAL need: small communities need fair, predictable access to shared water in the dry season.",
 "affected_population_or_system": "FICTIONAL small reservoir-sharing communities"
}
```
...
```text
FICTIONAL problem: the example community's reservoir has no agreed use schedule, and disputes rise in the dry months.
```
...
```text
FICTIONAL prior knowledge: written, locally agreed schedules are reported to reduce disputes in comparable settings.
```
...
```text
FICTIONAL gap: no study has tested a co-designed schedule in a community of this size and type.
```
...
```text
FICTIONAL RQ: does a co-designed written schedule reduce reported water-use disputes over one dry season?
```

`FUND.CALL.KEY_RESULTS` (ORIGIN: FUND_PROFILE; chain: KR; status: DRAFT; not on a tab because: a requirement of the bound call, not an NRIIS form field)
...
[
 {
  "id": "KR1",
  "code": "FICTIONAL-KR-1",
  "statement": "FICTIONAL key result: communities adopt a resource-sharing practice.",
  "numeric_target": 1,
  "target_unit": "practice",
  "target_period": "FICTIONAL year 1"
 }
]
```
...
[
 {
  "id": "OBJ1",
  "statement": "FICTIONAL: describe current water use and disputes.",
  "rq_ids": [
 ...
 {
  "id": "OBJ2",
  "statement": "FICTIONAL: co-design and test a shared schedule.",
  "rq_ids": [
   "CORE.RESEARCH.RQ.PRIMARY"
  ],
  "method_ids": [
...
{
 "paradigm": "pragmatic",
 "design_type": "FICTIONAL: before-after community study",
 "approach": "mixed methods",
 "quantitative_component": "Household survey before and after the dry season.",
 "qualitative_component": "Two co-design workshops.",
 "phases": [
  {
   "id": "MP1",
   "name": "Baseline description",
   "description": "Survey of current use and disputes.",
   "objective_ids": [
    "OBJ1"
   ]
  },
  {
   "id": "MP2",
   "name": "Co-design and test",
   "description": "Workshops, then a follow-up survey.",
   "objective_ids": [
    "OBJ2"
   ]
  }
...
```text
{
 "sampling_method": "census of households using the reservoir",
 "groups": [
  {
   "id": "SG1",
   "description": "FICTIONAL households using the reservoir",
   "target_n": 30
  }
 ],
 "sample_size_justification": "All households that use the reservoir (FICTIONAL count)."
}
```
...
[
 {
  "id": "INS1",
  "name": "FICTIONAL household water-use questionnaire",
  "purpose": "Measure use pattern and reported disputes.",
  "instrument_type": "questionnaire",
  "validity_method": "review by two lecturers",
  "reliability_method": "pilot with 5 households",
  "construct_measured": "reported water-use disputes"
 }
]
```

`CORE.RESEARCH.THEORETICAL_FOUNDATIONS` (ORIGIN: AUTHORING_CORE; chain: Framework; status: DRAFT; not on a tab because: authoring core, an internal research-design record (it reaches NRIIS only through the narrative box it feeds, if any); feeds: CORE.NARRATIVE.THEORY)

```text
FICTIONAL: common-pool resource governance, in the lecturer's own words.
```
...
```text
{
 ...
 "informed_consent": "Written consent from each household representative.",
 "privacy": "No names in the dataset; household codes only.",
 "confidentiality": "Data kept on the PI's encrypted laptop.",
 "withdrawal": "Any time, without reason.",
 "compensation": "None.",
 "risk_level": "minimal",
 "risk_management": "Workshops facilitated by a neutral co-researcher.",
 "vulnerable_groups": [],
...
[
 {
  "id": "DC1",
  "source": "FICTIONAL households (baseline)",
 ...
 {
  "id": "DC2",
  "source": "FICTIONAL households (workshops and follow-up)",
  "collection_method": "workshop notes and questionnaire",
  "timing": "months 5-9",
  "method_ids": [
   "MP2"
  ],
...
```text
{
 "quantitative_methods": [
  "before-after comparison of reported disputes"
 ],
 "qualitative_method": "thematic analysis of workshop notes",
 "software_or_tools": [
  "spreadsheet"
 ],
 "data_ids": [
...
```text
FICTIONAL evidence: a synthetic case study reports fewer disputes after a written schedule was agreed.
```
...
```text
FICTIONAL expected claim: a co-designed schedule is feasible in the example community.
```
...
| WC07 | PASS | CORE.ALIGNMENT.STATEMENT | The one-sentence alignment statement is written. |
...
| WC09 | OPEN | CORE.NARRATIVE.SUMMARY, CORE.NARRATIVE.RATIONALE, CORE.NARRATIVE.METHOD, CORE.RESEARCH.OBJECTIVES, CORE.ALIGNMENT.STATEMENT | No narrative box or core field is outside its length target. |
...
- No AI use is recorded in project.yaml (authoring.mode is human and no value is authored with AI assistance). If you used an AI tool anywhere in preparing this proposal, the guideline asks you to disclose it (p.10): fill authoring.ai_use_declaration.

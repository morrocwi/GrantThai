# Scored reading (DRAFT): the fictional demo against funded, public Thai research
...
**What this file does not do.** It does not judge the funded works or their
authors. A final report is not the proposal that was funded, and a
reading by keyword search can miss text. So this file (a) does not name
authors, (b) does not score a dimension from a failed search (that cell
is `OPEN`), and (c) states lessons as structure, not as flaws of a named
work. Specific page-level observations a reader might make about a named
report stay out of this public file until a human has re-checked them.
The blind scores belong in `docs/demo/comparison.md` section 4.
...
| R1 | funded final report on community strength on a rice-farming base (a synthesis with community co-researchers) | research core, D1–D8, D11 |
| R2 | funded final report on integrated community care for dependent older people | research core |
| R3 | funded final report synthesising earlier funded research on developing people, communities and society | research core |

Full citations (title, first author, contract or handle), hashes and access terms: `docs/demo/comparison.md` section 1 only.
R4 there is not used here: its licence and contents were not re-checked.
No source grants an open licence, so only short fragments are quoted, with
a page. No personal details or approval numbers from any source are copied.
...
| Ethics (part 3, standards) | OPEN (not found by full-text search of the final report; see D6) | two committees, consent, PDF p40–42 | consent and confidentiality, PDF p44–45 | consent route, vulnerable group, approval pending | N |
...
use), counting only parts found: R1 at least 10/12, R2 at least 10/12, R3
11/11 (sites not applicable), demo 11/12 (literature missing). A part not
found in a report is `OPEN`, not absent. Presence flatters the demo, because GrantThai asks
...
| D2 Gap | 2 | OPEN | OPEN | 4 | Demo gap is one local absence, AI-assisted, no prior work (`CORE.RESEARCH.GAP`). R1 and R2: not scored; the pages read did not settle it. R3 synthesises 5 funded programmes and 4 single projects (PDF p9) and names gaps between them as an output (PDF p22). |
| D3 RQ ↔ objectives | 3 | 5 | OPEN | 3 | Demo: 3 numbered objectives all point to the one primary RQ; secondary RQs are not linked. R1: 4 RQs and 4 objectives numbered one to one (PDF p23). R2: not scored; 3 objectives (PDF p19), and a report may state its questions differently from a proposal. R3: 1 RQ with sub-questions against 4 objectives (PDF p19–20). |
...
| D5 Method fit | 4 | 3 | 3 | 3 | Demo: design, sample, 4 instruments with validity and reliability notes, analysis, each tied to a phase and objective (`METHOD.PLAN.*`). R1: 5 PAR stages (PDF p39); per-objective instruments not verified. R2: descriptive design with a calculated sample size (PDF p38). R3: two visits per project and 4 synthesis forums (PDF p9); analysis rules not seen. |
| D6 Ethics / participants | 4 | OPEN | 4 | 4 | Demo: oral consent with a witness for elders who read poorly, coded names, risk to elders' knowledge credit; approval not yet sought. R1: not scored; no ethics or consent text was found by full-text search of the final report (terms searched: จริยธรรม, ยินยอม, พิทักษ์, ความลับ, consent), and a final report may omit ethics text its proposal had. R2: two committees, consent, withdrawal (PDF p40–42). R3: oral and signed consent, confidentiality (PDF p44–45). |
| D7 Output → outcome → impact | 3 | 3 | 3 | 4 | Demo: full chain with named users and a CONTRIBUTORY impact, but the only outcome has no baseline, target or period, and no key-result link. R1: expected results and a push to policy (PDF p25). R2: guidelines and proposals for agencies (PDF p90–93). R3: phased outputs, use plan, lead agencies and a 5-year horizon (PDF p7–8, p22–23). |
...
| D11 Epistemic honesty | 4 | N/A-report | N/A-report | N/A-report | Demo: 8 NEEDS_VERIFICATION markers, 5 NEEDS_INPUT, nothing above DRAFT; but simulated answers are stored as `authored_by: human` (G8), and "Submittable: yes" refers to a fictional call (G12). R1–R3: cite their figures; a final report has no reason to carry proposal-stage uncertainty markers, so this dimension says little about them. |

**Totals over the five dimensions scored for all four (D1, D4, D5, D7,
D8):** demo 15, R1 19, R2 18, R3 18 (`INSTINCT`, one reader). D2, D3 and
D6 have `OPEN` cells and are left out. D11 was designed around GrantThai's own markers, so it favours the
...
3. **A framework with parts.** R2 and R3 name constructs and their origin.
   ...
   stored constructs.
4. **Position against earlier funded work.** R3 is built on 5 funded
   programmes and 4 single projects (PDF p9). The demo cites nothing funded. R0 asks for this only under
...
8. **Key-result alignment.** R0 asks for the fund's key results (part 1,
   p1). The demo honestly says NEEDS_INPUT, so it cannot compete where a
   funder looks first.

## 5. What GrantThai enforces that a final report does not show

These are structural differences between a proposal worksheet and a final
report, not flaws of the funded works. The proposals behind R1–R3 are not
public, so nothing here says what their authors did or did not do.

- **Provenance per value.** Every demo value cites its transcript turn. A
  report cites sources for figures but has no per-value record of who
  wrote what.
- **Gaps stay visible.** The demo prints NEEDS_INPUT and
  NEEDS_VERIFICATION; a finished report has no such marker.
- **Internal consistency checks.** Budget arithmetic and activity weights
  are recomputed by rules. Long documents written by hand have no such
  check, so small mismatches between sections can pass unnoticed.
...
| 2 | New REVIEW rules: outcome without baseline, target or measurement period; outcome dated after project end; activity without deliverable or output; output linked to an activity that does not produce it; a dependent activity that starts before its predecessor ends (the demo's ACT2 starts in month 2 while ACT1, which it depends on, runs months 1–2) | `validators/rules.yaml`, engine, `tests/` | the demo's D7 and workplan flaws pass silently |
...
| 10 | Run the blind two-scorer round, and have a human re-check every page cited here | `docs/demo/comparison.md` section 4 | this reading is not blind and not independent of GrantThai's design |

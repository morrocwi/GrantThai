...
| `grantthai_list_structure_profiles` | `grantthai route profiles` | Lists the 7SSA structure profiles of the academic-article route (visible sections, heading language, paired sub-profile) and, with `project_path`, what the researcher selected and the router's candidates. Never selects and never writes `routing.structure_profiles`. `grantthai_validate`, `grantthai_check_route` and `grantthai_build` take an optional `structure_profile` the researcher chose; `grantthai_build` also takes `format` (`md` or `tex`: `build/ACADEMIC_ARTICLE.tex` instead of the Markdown file) and `glosa_audit`. |
